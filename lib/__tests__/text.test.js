...
import { linkify, nl2br, truncate, mark, parseTag } from '../text';
...
describe('mark', () => {
  ...
  it('does nothing when no textToMatch, or when textToMatch is empty string', () => {
    expect(mark('foo')).toBe('foo');
    expect(mark('foo', { stringsToMatch: [] })).toBe('foo');
    expect(mark('foo', { stringsToMatch: [''] })).toBe('foo');
  });

  it('does nothing if no match at all', () => {
    expect(mark('foo', { stringsToMatch: ['bar'] })).toBe('foo');
  });
});

describe('parseTag', () => {
  it('hanldes HIGHLIGHT parsing', () => {
    expect(parseTag('foo <HIGHLIGHT>Foo!</HIGHLIGHT> bar')).toMatchSnapshot();
  });
  it('does nothing when no tags', () => {
    expect(parseTag('foo')).toBe('foo');
    expect(parseTag('foo <NotMatching>bar</NotMatching>')).toBe(
      'foo <NotMatching>bar</NotMatching>'
    );
  });
});
