...
const urlRegExp = RegExp(`(${urlRegex().source})`, 'gi');
 ...
 * @param {<maxLength: Number, props: Objecct>} options
 */
 ...
 *
 * @param {*} elem React element, string, array of string & react elements
 * @param {<stringsToMatch: string[], props: object>} options
 */
export function mark(elem, { stringsToMatch = [], props = {} } = {}) {
  const strings = stringsToMatch.filter(s => !!s);
  if (strings.length === 0) return elem;
  const textToMatchRegexp = new RegExp(
  ...
  return traverseForStrings(elem, str => {
    ...
    const tokenized = str.split(textToMatchRegexp).map((s, i) =>
      s.match(textToMatchRegexp) ? (
        <mark key={`mark${i}`} {...props}>
          {s}
        </mark>
      ) : (
        s
...
/**
 * @param {*} elem React element, string, array of string & react elements
 * @param {<fromElem: string, toTag: string, toElemProps: object>} options
 */
export function parseTag(
  elem,
  { fromElem = 'HIGHLIGHT', toElem = 'span', toElemProps = {} } = {}
) {
  const startTag = `<${fromElem}>`;
  const endTag = `</${fromElem}>`;
  const elemMatcher = new RegExp(`(${startTag}|${endTag})`, 'im');
  const Elem = toElem;

  return traverseForStrings(elem, str => {
    if (!str) return str;

    let elemContent = null; // null: Not in element; array: currently in element
    const tokenized = [];

    str.split(elemMatcher).forEach((s, i) => {
      switch (s) {
        case startTag:
          elemContent = [];
          break;
