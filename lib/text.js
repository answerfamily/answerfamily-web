 ...
 *
 * @param {*} elem React element, string, array of string & react elements
 * @param @param {<stringsToMatch: string[], getProps: function>} options
 */
export function mark(elem, { stringsToMatch = [], getProps = () => ({}) }) {
  if (stringsToMatch.length === 0) return elem;
  const textToMatchRegexp = new RegExp(
    `(${stringsToMatch.map(escapeStringRegexp).join('|')})`,
    'im'
  );
